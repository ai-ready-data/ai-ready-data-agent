...
|---------|---------|------------|
| **assess** | Full pipeline: discover → run → report → [save] → output | `-c`, `--connection` (or `AIRD_CONNECTION_STRING`), `-o` (markdown \| stdout \| json:path), `--no-save`, `--compare`, `--dry-run`, `--context`, `--suite`, `-i`/`--interactive`, `--audit` |
